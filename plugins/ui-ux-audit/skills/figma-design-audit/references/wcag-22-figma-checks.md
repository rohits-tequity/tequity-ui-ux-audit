...
| 1.3.2 Meaningful Sequence | A | D | Compare DOM/layer order against visual order sorted by (y, x). A mismatch is a reading-order risk, flag for runtime confirmation. |
...
| 1.4.3 Contrast (Minimum) | AA | D | Computed: ≥4.5:1 body text, ≥3:1 for ≥18.66px bold or ≥24px regular. Text over an image or gradient is **indeterminate** → mark Not Evaluated and require a runtime pixel probe. |
...
| 2.5.8 Target Size (Minimum) | AA (2.2) | D | ≥24×24 CSS px, or smaller with ≥24px spacing between target centres. Platform minima are stricter (see platform-guidelines.md), report against both and grade on the stricter. |
...
| SC | Level | Where | Design-stage check |
|---|---|---|---|
| 3.1.1 / 3.1.2 Language | A/AA | D | Language of page and of any foreign-language passage declared. |
...
| 4.1.3 Status Messages | AA | D | Toasts, inline validation, loading and "3 results found" need a live-region spec. |
